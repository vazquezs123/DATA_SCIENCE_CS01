...
##Which stat has the most bitter (Highest IBU) beer?
```{r mostIBU}
#The highest ABV is:
...
ggplot(beersAndBreweries, aes(x = ABV)) +
    ...
    theme(panel.background = element_rect(fill = "#FDF8E2"))
```
A boxplot is a better way to see all of this.

#Is there a correlation between IBU and ABV?
The client asked for a scatter plot, but a linear model and correlation percentage helps here too!
```{r correlateABVIBU}
...
```
As we can see, there is a 67% correlation between ABV and IBU. The plot also visually suggests that generally the more bitter a beer is, the higher the alcohol per volume.
